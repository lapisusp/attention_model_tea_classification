...
for b=1:length(ind)
    ...
    %imshow(absOutMeanResized);
    % Fill in features
    features(:, b) = absOutMeanResized(:);
     nome = strcat('subband',int2str(b),'.jpg');
     imwrite(absOutMeanResized, nome);
end

if nargout<1
    for b = 1:14
        subband = reshape(features(:, b), dims);
        subplot(4, 4, b); imagesc(subband); title(strcat('band=', num2str(b))); %colorbar;
    end
end