...
features=zeros(dims(1)*dims(2), 6);
img=im2double(img);

% calculate each color channel and the probability of that color
for i=1:3
    channel=img(:, :, i);
    channel=imresize(channel, dims);
    features(:, i)=channel(:);
%     nome = strcat('colorChanel',int2str(i),'.jpg');
    % imwrite(channel, nome);
    ...
    probChannel=imresize(probChannel, dims);
    features(:, i+3)=probChannel(:);
%     nomea = strcat('colorProb',int2str(i+3),'.jpg');
    % imwrite(probChannel, nomea);
end
...
%     featureMap=imresize(featureMap, dims);
%     features(:, i+6)=featureMap(:);
%     nomec = strcat('colorProbBlur',int2str(i),'.jpg');
%     % imwrite(featureMap, nomec);
% end
% fprintf([num2str(toc), ' seconds \n']);
% 
